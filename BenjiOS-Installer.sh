...
show_advanced_options() {
    ADVANCED_ACTIONS=$(zenity --list --checklist --title="BenjiOS – Advanced Options" \
        --width="$ZENITY_W" --height=400 \
        --text="Select advanced actions to perform:" \
        --column="Select" --column="Action" --column="Description" \
        FALSE "reenroll_refind_key" "Re-enroll Secure Boot key for rEFInd" \
        FALSE "reapply_gnome_theme" "Reapply Yaru Olive GNOME theme" \
        FALSE "reapply_terminal_config" "Reapply Terminal (Ptyxis) configuration" \
        FALSE "reapply_refind_config" "Reapply rEFInd config and theme")

    [ -z "$ADVANCED_ACTIONS" ] && return

    IFS="|" read -ra ACTIONS <<< "$ADVANCED_ACTIONS"
    for act in "${ACTIONS[@]}"; do
        case "$act" in
            ...
            reapply_gnome_theme)
                ...
                gsettings set org.gnome.desktop.wm.preferences theme 'Yaru-olive-dark'
                ;;
            reapply_terminal_config)
                if command -v dconf >/dev/null 2>&1; then
                    tmp_term="$(mktemp)"
                    if curl -fsSL "$RAW_BASE/configs/terminal.conf" -o "$tmp_term"; then
                        dconf load /org/gnome/Ptyxis/ < "$tmp_term"
                    fi
                    rm -f "$tmp_term"
                fi
                ;;
            reapply_refind_config)
                REFIND_FORCE_THEME_ONLY=true
                ;;
        esac
    done
}
